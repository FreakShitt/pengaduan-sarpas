...
<body>
    ...
    <main>
        ...
        <section class="mono-section" style="padding: 6rem 0 4rem; border-bottom: 2px solid black;">
            <div class="mono-container">
                <h1 style="font-size: 4rem; margin-bottom: 1rem;">EDIT LOKASI</h1>
                <p style="font-size: 1.25rem; color: var(--color-gray-600); max-width: 600px;">
                    Edit data lokasi "{{ $lokasi->nama_lokasi }}"
                </p>
            </div>
        </section>
                ...
                <form action="{{ route('admin.lokasi.update', $lokasi->id) }}" method="POST">
                    ...
                    <div class="mono-form-group">
                        ...
                        <textarea 
                            ...
                            name="deskripsi" 
                            class="mono-textarea"
                            placeholder="Deskripsi detail lokasi...">{{ old('deskripsi', $lokasi->deskripsi) }}</textarea>
                    </div>

                    <div class="mono-form-group">
                        <label class="mono-label" for="aktif">Status</label>
                        <select id="aktif" name="aktif" class="mono-select" required>
                            <option value="1" {{ old('aktif', $lokasi->aktif) == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('aktif', $lokasi->aktif) == '0' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                    </div>
